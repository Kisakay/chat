...
| GET | `/api/tools/tts/info` | Bearer | neural TTS provider + voices (null = browser fallback) |
| POST | `/api/tools/tts` | Bearer | synthesize `{text, voice?}` → audio bytes (cached, 30/hour) |
...
| WS | `/api/admin/ws?token=…` | admin | firehose of all access + report events (badges + triage lists, full payloads, no polling) |
