...
const STATUS_STYLE: Record<AccessRequest["status"], { cls: string; icon: typeof Clock }> = {
  pending: { cls: "bg-amber-500/10 text-amber-700 dark:text-amber-400", icon: Clock },
  reviewing: { cls: "bg-sky-500/10 text-sky-700 dark:text-sky-400", icon: Search },
  accepted: { cls: "bg-accent-500/10 text-accent-700 dark:text-accent-400", icon: CheckCircle2 },
  refused: { cls: "bg-red-500/10 text-red-600 dark:text-red-400", icon: XCircle },
};
  ...
  return (
    <div className="grid min-h-full place-items-center bg-gradient-to-br from-stone-100 via-stone-50 to-accent-100/60 p-4 dark:from-zinc-950 dark:via-zinc-950 dark:to-accent-950/40 sm:p-6">
      <div className="w-full max-w-xl rounded-[2rem] border border-white/60 bg-white/80 p-6 shadow-2xl backdrop-blur-xl dark:border-zinc-700/60 dark:bg-zinc-900/80 sm:p-8">
        ...
        <h1 className="text-center font-serif text-2xl font-bold tracking-tight">{t("review.title")}</h1>

        {loading && <p className="mt-4 flex items-center justify-center gap-2 text-sm opacity-60"><Spinner size={15} /> {t("review.loadingTicket")}</p>}

        {invalid && (
          <div className="mt-4 space-y-3 text-center">
            <p className="flex items-center justify-center gap-2 font-semibold"><MailWarning size={18} className="text-red-500" /> {t("review.notFound")}</p>
          ...
          <div className="mt-4 space-y-4">
            ...
            {request.reason && (
              <p className="rounded-2xl bg-stone-100 px-4 py-2.5 text-center text-sm dark:bg-zinc-800">
                <span className="font-medium">{t("review.adminNote")} </span>{request.reason}
              </p>
            )}
              ...
              {messages.map((m) => (
                <div key={m.id} className={cn("flex", m.author === "admin" ? "justify-start" : "justify-end")}>
                  <div className={cn(
                    ...
                    <p className="mb-0.5 text-[11px] font-medium opacity-70">{m.author === "admin" ? t("review.admin") : t("review.you")}</p>
...
export function AccessRequestModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  return (
    <Modal open={open} onClose={onClose} title={t("review.requestTitle")} icon={MessageSquareText}>
      {done ? (
        ...
        <form onSubmit={submit} className="space-y-4">
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" type="submit" disabled={busy}>{busy ? <Spinner size={15} /> : t("review.sendRequest")}</Button>
          </div>
        </form>
      )}
    </Modal>
