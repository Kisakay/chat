...
import { CheckCircle2, Clock, MailWarning, MessageSquareText, Search, SendHorizontal, XCircle } from "lucide-react";
import { api, type AccessMessage, type AccessRequest } from "../lib/api.ts";
import { Button, CopyButton, Field, FlowerMark, Input, Modal, Spinner } from "./ui.tsx";
import { cn } from "../lib/cn.ts";

const STATUS_STYLE: Record<AccessRequest["status"], { label: string; cls: string; icon: typeof Clock }> = {
  pending: { label: "Pending review", cls: "bg-amber-500/10 text-amber-700 dark:text-amber-400", icon: Clock },
  reviewing: { label: "Under review", cls: "bg-sky-500/10 text-sky-700 dark:text-sky-400", icon: Search },
  accepted: { label: "Accepted", cls: "bg-accent-500/10 text-accent-700 dark:text-accent-400", icon: CheckCircle2 },
  refused: { label: "Refused", cls: "bg-red-500/10 text-red-600 dark:text-red-400", icon: XCircle },
};

function StatusBadge({ status }: { status: AccessRequest["status"] }) {
  const s = STATUS_STYLE[status];
  return (
    <span className={cn("inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-medium", s.cls)}>
      <s.icon size={13} />
      {s.label}
    </span>
  );
      ...
      <div className="w-full max-w-xl rounded-[2rem] border border-white/60 bg-white/80 p-6 shadow-2xl backdrop-blur-xl dark:border-zinc-700/60 dark:bg-zinc-900/80 sm:p-8">
        ...
        {invalid && (
          <div className="mt-4 space-y-3 text-center">
            <p className="flex items-center justify-center gap-2 font-semibold"><MailWarning size={18} className="text-red-500" /> Ticket not found</p>
            <p className="text-sm opacity-70">This link is unknown. Check the URL from your email, or file a new request from the login page.</p>
          ...
          <div className="mt-4 space-y-4">
            ...
            {request.reason && (
              <p className="rounded-2xl bg-stone-100 px-4 py-2.5 text-center text-sm dark:bg-zinc-800">
                <span className="font-medium">Admin note: </span>{request.reason}
              </p>
            )}
            ...
            <div className="max-h-80 space-y-2.5 overflow-y-auto rounded-2xl border border-stone-200/70 p-3 dark:border-zinc-800">
              {messages.map((m) => (
                <div key={m.id} className={cn("flex", m.author === "admin" ? "justify-start" : "justify-end")}>
                  <div className={cn(
                    ...
                    <p className="mb-0.5 text-[11px] font-medium opacity-70">{m.author === "admin" ? "Admin" : "You"}</p>
              ...
              <form onSubmit={send} className="space-y-2">
                <div className="flex items-end gap-2">
                  <textarea
                    ...
                    aria-label="Reply to your ticket"
                  ...
                  >
                    {sending ? <Spinner size={16} /> : <SendHorizontal size={16} />}
                  </button>
                </div>
...
export function AccessRequestModal({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const [error, setError] = useState("");
  const [done, setDone] = useState<{ reviewUrl: string; username: string } | null>(null);
  ...
  return (
    <Modal open={open} onClose={onClose} title="Request access" icon={MessageSquareText}>
      {done ? (
        <div className="space-y-4 text-center">
          ...
          <p className="text-sm opacity-70">
            @{done.username} is reserved for you while an admin reviews it.
            A confirmation email with your personal ticket link is on its way.
          </p>
          <div className="flex items-center gap-2">
            <code className="min-w-0 flex-1 truncate rounded-2xl bg-stone-100 px-3 py-2.5 text-left text-xs dark:bg-zinc-800">{done.reviewUrl}</code>
            <CopyButton text={done.reviewUrl} label="Copy link" />
          </div>
          <Button size="sm" className="w-full" onClick={() => (window.location.href = done.reviewUrl.replace(window.location.origin, ""))}>
            Open my ticket
          </Button>
        </div>
      ) : (
        <form onSubmit={submit} className="space-y-4">
          <p className="text-sm opacity-70">
            Reserve a username and tell us why you want in. An admin reviews every request —
            you follow the discussion on your personal ticket page and by email.
          </p>
          <Field label="Username">
            <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="alice" required minLength={2} maxLength={32} />
          </Field>
          <Field label="Email" hint="Required — status updates and ticket replies land here.">
            <Input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="alice@example.com" inputMode="email" required />
          </Field>
          <Field label="Why should you get access?" hint="10+ characters. This starts your ticket thread.">
            <textarea
              value={message}
          ...
          <div className="flex justify-end gap-2">
            ...
            <Button size="sm" type="submit" disabled={busy}>{busy ? <Spinner size={15} /> : "Send request"}</Button>
          </div>
        </form>
      )}
    </Modal>
