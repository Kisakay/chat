...
function AttachmentBlock({ header, body }: { header: string; body: string }) {
  const [open, setOpen] = useState(false);
  const label = header.startsWith("[") && header.endsWith("]") ? header.slice(1, -1) : header;
  return (
    <span className="mt-2 block overflow-hidden rounded-2xl bg-black/25 text-left dark:bg-black/30">
      <button
        type="button"
        onClick={() => setOpen((v) => !v)}
        aria-expanded={open}
      ...
      {open && (
        <pre className="max-h-72 overflow-auto whitespace-pre-wrap break-words border-t border-white/10 px-3 py-2 font-mono text-[12.5px] leading-relaxed">
          {body}
        </pre>
      )}
    </span>
...
export function UserMessageContent({ content }: { content: string }) {
  ...
  while ((m = ATTACH_RE.exec(content)) !== null) {
    ...
    parts.push(<AttachmentBlock key={key++} header={m[1]!} body={m[2] ?? ""} />);
